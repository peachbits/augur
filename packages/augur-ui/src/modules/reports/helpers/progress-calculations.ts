...
export const calculatePercentage = (numSize, numTotalStake) => {
  ...
  const value = ONE.minus(ratio)
    .times(ONE_HUNDRED)
    .toFixed(4, 1);
  return createBigNumber(value).toNumber();
};

export const calculateNonAccountPercentage = (
  size,
  numStakeCurrent,
  numAccountStakeCurrent
) => {
  // NB: Remove whence paramaters are converted outside
...
export const calculateAddedStakePercentage = (
  size,
  numAccountStake,
  numAddedStake
) => {
...
export const calculateTentativeCurrentRep = (
  numTotalStake,
  numTentativeStake
) => {
  // NB: Remove whence paramaters are converted outside
  ...
  const result = totalStake
    .plus(convertRepToAttoRep(tentativeStake))
    .toNumber();
