...
import { PlaceTradeDisplayParams } from "./Trade";
...
export class ContractAPI {
  ...
  public async approveCentralAuthority(): Promise<void> {
    ...
    await this.augur.contracts.cash.approve(authority, new BigNumber(2).pow(256).minus(1));
  }
  ...
  public async createYesNoMarket(
    ...
    const byteTopic = formatBytes32String(topic);
    ...
    const marketId = marketIdFromEvents(createMarketEvents);
  ...
  public async createCategoricalMarket(
    ...
    const byteTopic = formatBytes32String(topic);
    const createMarketEvents = await universe.createCategoricalMarket(
      endTime,
  ...
  public async createScalarMarket(
    ...
    const createMarketEvents = await universe.createScalarMarket(
      endTime,
      feePerCashInAttoCash,
      affiliateFeeDivisor,
      designatedReporter,
  ...
  public async createReasonableScalarMarket(universe: ContractInterfaces.Universe): Promise<ContractInterfaces.Market> {
    ...
    const fee = new BigNumber(10).pow(16);
    const affiliateFeeDivisor = new BigNumber(25);
    const minPrice = new BigNumber(50).times(10).pow(18);
    const maxPrice = new BigNumber(250).times(10).pow(18);
  ...
  public async placeBasicYesNoTrade(direction: 0 | 1, market: ContractInterfaces.Market, outcome: 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7, displayAmount: BigNumber, displayPrice: BigNumber, displayShares: BigNumber): Promise<void> {
    await this.placeTrade({
      ...
      displayPrice,
      displayShares,
      onSuccess: (r) => { console.log(`SUCCESS: ${JSON.stringify(r)}`); },
      onFailed: (r) => { console.log(`FAIL: ${r}`); },
      onSent: (r) => { console.log(`SENT: ${JSON.stringify(r)}`); },
    });
  }
  ...
  public async getEthBalance(): Promise<BigNumber> {
    return new BigNumber(await this.provider.getBalance(this.account).toString());
  }
...
function marketIdFromEvents(createMarketEvents: Array<ContractInterfaces.Event>): string {
  for (const ev of createMarketEvents) {
    if (ev.name === "MarketCreated") {
      interface HasMarket {
        market: string;
      }
      return (ev.parameters as HasMarket).market;
    }
  }

  throw Error("Logs do not include CreateMarketEvent");
}
