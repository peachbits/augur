...
async function backfillCategoryOpenInterestFromMarkets(knex: Knex, marketRows: MarketRow[], categoryColumnName: "openInterest" | "nonFinalizedOpenInterest"): Promise<void> {
...
exports.up = async (knex: Knex): Promise<any> => {
  ...
  if (!doesOpenInterestExist) {
    ...
    const marketRows: MarketRow[] = await knex.select(["category", "openInterest"]).from("markets");
  ...
  if (!doesNonFinalizedOpenInterestExist) {
    ...
    const marketRows: MarketRow[] = await knex.select([
